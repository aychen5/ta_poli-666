...
mod.p <- glm(abd ~ age + fthr.ed + mthr.ed + C.ach, data = d, family = binomial(link = "logit"))
...
# having C.ach=1 instead of C.ach=0, conditional on other
# covariates
...
lalonde_match <- data.frame(Z = matched_gen$mdata$Tr,
                        Y = matched_gen$mdata$Y)

complete_dec <- declare_ra(N = 370, m = 185)
...
d <- read.csv(file.path(path, "child_soldiering.csv"))

# Use the regression estimator
mod1 <- lm(educ ~ abd + age + fthr.ed + mthr.ed + C.ach, data = d)
mod1HC2 <- list(coeftest(mod1, vcov = vcovHC(mod1, type = "HC2"))[,
2])
...
# Make plot
ggplot(data.frame(delta = delta, g = g), aes(x = delta, y = g)) +
geom_path() + xlab(expression(delta)) + ylab(expression(gamma)) +
ylim(-3, 0)

### OR.....
#Create a contour plot of the bias that would reduce themagnitude of the ATE by hal
b.half <- abs(lm.beta(mod1)$standardized.coef[2]/2)

# The range of delta is 0,1 because delta = Pr(Ui = 1|Di = 1)−Pr(Ui = 1|Di = 0)
delta <- seq(0.001, 1, by = 0.01)

# Solve for gamma. E[Yi|Ui =1]−E[Yi|Ui =0]
g <- b.half/delta

##These are the values of delta and gamma that would cut out ATE in half
plot(x = delta, y = g, ylim = c(0,1), xlim = c(0,1), type='l')
```
